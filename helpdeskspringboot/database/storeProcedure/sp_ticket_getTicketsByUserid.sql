...
DROP PROCEDURE IF EXISTS sp_ticket_getTicketsByUserid;
...
-- customer: call sp_ticket_getTicketsByUserid(1)
-- supporter: call sp_ticket_getTicketsByUserid(2)
-- admin: call sp_ticket_getTicketsByUserid(3)

CREATE PROCEDURE sp_ticket_getTicketsByUserid(in_userid int, searchTerm varchar(2000))
BEGIN
...
if userRole = "ROLE_CUSTOMER" then
	...
	select 	a.ticketid, 
			a.subject,
			concat(coalesce(b.lastName,''),' ', coalesce(b.firstName,'')) as assigneeName
    from ticket a
		left join user b on coalesce(a.assigneeid,'') = b.id
